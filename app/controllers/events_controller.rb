class EventsController < ApplicationController
  ...
  def get_events
    @events = current_project.events.where('starttime  >= :start_time and 
                          endtime     <= :end_time',
  ...
  def load_event
    @event = current_project.events.where(:id => params[:id]).first
    unless @event
  ...
  def event_params
    params.require(:event).permit('title', 'description', 'starttime', 'endtime', 'all_day', 'period', 'frequency', 'commit_button', 'project_id')
  end
